/**
 * TRUNKIA Sovereign Protocol v19.0 (Omega Protocol - Cloud Edition)
 * Features: Distributed PII (Redis), Mid-Stream Failover, Deterministic Grounding.
 */
...
class SovereignProtocol {
  ...
  async *executeStream(prompt, taskType = 'general', userId = 'global', abortSignal = null) {
    ...
    // OMEGA: Deep decode prompt before processing (prevents encoding bypass)
  const promptDecode = deepDecode(baitedPrompt);
  if (promptDecode.rejected) {
    yield { type: 'error', content: 'SOVEREIGN VIOLATION: Obfuscated input rejected (' + promptDecode.reason + ').', metadata: { decodedLayers: promptDecode.layers } };
    return;
  }
  ...
  const { sanitizedText, mapping } = await piiRedactor.redact(decodedPrompt, sessionId);
    ...
    for (const provider of chain) {
      ...
      try {
        ...
        if (e.name === 'AbortError') { if (abortSignal?.aborted) break; }
      } finally {
  ...
  const { sanitizedText, mapping } = await piiRedactor.redact(promptDecode.decoded, sessionId);
