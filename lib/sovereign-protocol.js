/**
 * TRUNKIA Sovereign Protocol v22.0 (Omega Protocol - Enterprise Edition)
 * Integrated: QuotaContext (Metered Billing), Deep Decoder, PII Vault, Truth Tribunal.
 */
...
import { createQuotaContext } from './services/quota-manager.js';
...
class SovereignProtocol {
  ...
  async *executeStream(prompt, taskType = 'general', userId = 'global', abortSignal = null, quotaContext = null) {
    ...
    let rollingContext = '', fullTextForGrounding = '', quotaExhausted = false;
    ...
    for (const provider of chain) {
      ...
      try {
        ...
        if (buffer.length > 0 && !canaryLeaked && !quotaExhausted) { const r = recon.process(buffer); if (r) yield { type: 'chunk', content: r }; }
        const finalFlush = recon.flush();
        if (finalFlush) yield { type: 'chunk', content: finalFlush };
        if (canaryLeaked || quotaExhausted) break;
  ...
  async execute(prompt, taskType = 'general', userId = 'global', quotaContext = null) {
    const sessionId = crypto.randomUUID();
