/**
 * TRUNKIA Sovereign Protocol v18.0 (Omega Protocol)
 * Features: Deterministic Grounding v2.0, Bounded Mid-Stream Failover, PII Vault v13.0.
 */
...
class SovereignProtocol {
  static TASK_CONFIG = {
    general: { maxTokens: 2048, temperature: 0.3, systemPrompt: 'You are TRUNKIA Sovereign Intelligence Core. Respond with precision. If uncertain, state your uncertainty explicitly. Do not fabricate sources or quotes. Distinguish facts from inferences.' },
    financial: { maxTokens: 4096, temperature: 0.1, systemPrompt: 'You are TRUNKIA Financial Intelligence. Respond with extreme precision. No speculation. If data is unavailable, state so. Never fabricate financial figures.' },
  ...
  async *executeStream(prompt, taskType = 'general', userId = 'global', abortSignal = null) {
    ...
    // === DETERMINISTIC GROUNDING v2.0 (Post-Stream) ===
    const groundingResult = groundingEngine.ground(fullTextForGrounding);

    const actualCost = inputTokens + outputTokens;
    const metadata = { inputTokens, outputTokens, totalCost: actualCost, model: modelUsed, providersAttempted: providerAttempted, providerSucceeded, canaryLeaked };
  ...
  async execute(prompt, taskType = 'general', userId = 'global') {
    ...
    let finalContent = piiRedactor.reconstruct(primaryResponse.content, mapping);

    // === DETERMINISTIC GROUNDING v2.0 ===
    ...
    semanticCache.store(prompt, { content: finalContent }, userId, 0, { verifiable: true, grounding });
