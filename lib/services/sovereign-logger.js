  ...
  _log(level, msg, meta, overrideStore) {
    ...
    try {
      if (level === 'error' || level === 'fatal') {
        var errW = process.stderr._origWrite || process.stderr.write.bind(process.stderr); errW(serialized + '\n');
      } else {
        var outW = process.stdout._origWrite || process.stdout.write.bind(process.stdout); outW(serialized + '\n');
      }
    } finally {
      isWriting = false;
    }
