...
class Agent {
  constructor() { this.name = 'ab_testing_agent'; this.layer = 'service'; this.status = 'active'; }
  async initialize() { try { await pool.query('SELECT 1'); return true; } catch(e) { this.status='db_error'; return false; } }
  async run(input = {}) {
    try {
      const result = await safeGroqJSON(`أنت وكيل اختبار A/B. حلل نتائج الاختبارات وقارن الأداء بين الإصدارات وأعط توصيات للنسخة الأفضل. البيانات: ${JSON.stringify(input)}. أجب بـ JSON: {analysis:string,recommendations:array,confidence:number,action_taken:string}`);
...
export { agent as abTestingAgent };
