...
class Agent {
  constructor() { this.name = 'cdn_agent'; this.layer = 'service'; this.status = 'active'; }
  async initialize() { try { await pool.query('SELECT 1'); return true; } catch(e) { this.status='db_error'; return false; } }
  async run(input = {}) {
    try {
      const result = await safeGroqJSON(`أنت وكيل شبكة CDN. راقب أداء التوزيع الجغرافي وزمن التحميل واكتشف نقاط الاختناق. البيانات: ${JSON.stringify(input)}. أجب بـ JSON: {analysis:string,recommendations:array,confidence:number,action_taken:string}`);
      if (!result.data) return { success:false, error:result.error };
      try {
        await pool.query(
          `INSERT INTO agent_execution_logs (agent_name,action,input,output,confidence,status) VALUES ($1,'cdn',$2,$3,$4,'completed')`,
