...
const pool = getPool('main', generateDbToken('lib/services/executor.js'));;
import { mkdir } from 'fs/promises';
import { existsSync } from 'fs';
import { safeIdentifier } from './db-safety.js';
...
export async function safeInsert(table, data) {
  ...
  try {
    ...
    // تحقق فعلي
    const verify = await pool.query(`SELECT id FROM ${safeIdentifier(table)} WHERE id=$1`, [rows[0].id]);
