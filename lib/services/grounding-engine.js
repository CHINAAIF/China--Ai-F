/**
 * TRUNKIA Deterministic Grounding Engine v3.0 (Omega Protocol - Global Edition)
 * Integrated: Deep Decoder v2.0 (Encoding Bypass Prevention)
 */
import { deepDecode } from './deep-decoder.js';
...
class GroundingEngine {
  ...
  stripCodeBlocks(text) {
    return text.replace(/```[\s\S]*?```/g, ' [CODE_BLOCK] ').replace(/`[^`]*`/g, ' [CODE_INLINE] ');
  }
  ...
  ground(text) {
    if (!text || typeof text !== 'string' || text.trim().length === 0) {
      return { grounded: true, confidence: 100, contradictions: [], claims: 0, fabrication: [], warnings: [], decodedLayers: [] };
    }

    // OMEGA: Deep Decode before analysis
    const decodeResult = deepDecode(text);
    if (decodeResult.rejected) {
      return {
        ...
        claims: 0,
        fabrication: [],
        warnings: ['DECODE_REJECTED: ' + decodeResult.reason],
        decodedLayers: decodeResult.layers
      };
    }
