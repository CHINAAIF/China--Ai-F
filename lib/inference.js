...
// ─────────────────────────────────────────────────────────────
// PURE SOVEREIGN CACHE (Zero-Dependency Keyword & Trigram Matching)
// ─────────────────────────────────────────────────────────────
...
async function checkSemanticCache(client, prompt) {
  ...
  // 1. Exact Keyword Array Match (Fastest)
  ...
  // 2. Fuzzy Trigram Match (Handles different wordings)
...
async function saveToSemanticCache(client, prompt, response) {
  const keywords = extractKeywords(prompt);
...
export async function executeInference(messages, taskType) {
  ...
  try {
    ...
    // 1.5 Sovereign Semantic Cache Check
    ...
    for (const model of modelsToTry) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          saveToSemanticCache(client, promptText, data.choices[0].message.content);
