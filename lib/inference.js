...
export async function analyzeWithPythonSidecar(messages) {
  const sidecarUrl = 'http://127.0.0.1:8001/analyze';
  ...
  try {
    ...
    const timeout = setTimeout(() => controller.abort(), 500); // 150ms hard timeout
    ...
    if (!response.ok) throw new Error('SIDECAR_HTTP_ERROR');
    return await response.json();
    
  } catch (err) {
    console.error('[SIDECAR_DOWN] Falling back to Regex Shield:', err.message);
