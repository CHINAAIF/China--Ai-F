...
// ─────────────────────────────────────────────────────────────
// KNOWLEDGE GROUNDING LAYER (RAG - Retrieval-Augmented Generation)
// ─────────────────────────────────────────────────────────────
async function getGroundedContext(client, prompt) {
  try {
    const res = await client.query(
      "SELECT id, topic, content::text as text_content FROM platform_knowledge_base WHERE similarity(topic, $1) > 0.2 OR similarity(content::text, $1) > 0.2 ORDER BY confidence_score DESC LIMIT 2",
      [prompt]
    );
    
    if (res.rows.length > 0) {
      // Update times_used asynchronously
      const ids = res.rows.map(r => r.id);
      client.query("UPDATE platform_knowledge_base SET times_used = times_used + 1 WHERE id = ANY($1::uuid[])", [ids]).catch(() => {});
      
      // Build context string
      const contextStr = res.rows.map(r => r.topic + ": " + r.text_content).join("\n");
      return contextStr;
    }
  } catch (e) {
    console.error('[KB_SEARCH_ERR]', e.message);
  }
  return null;
}
    ...
    for (const model of modelsToTry) {
      ...
      try {
        const response = await fetch(providerConfig.url, {
          ...
          // Knowledge Grounding: Inject context if found
          const groundedContext = await getGroundedContext(client, promptText);
          let finalMessages = [...messages];
          if (groundedContext) {
            console.log('[KB_HIT] Grounding context injected.');
            finalMessages.unshift({
              role: 'system',
              content: "You must answer strictly based on this internal TRUNKIA context. Do not hallucinate or invent information outside this context:\n" + groundedContext
            });
          }
