...
class Agent {
  ...
  async run(input = {}) {
    try {
      const result = await safeGroqJSON(`أنت وكيل مراجعة الجودة. افحص مخرجات الوكلاء الأخرى وقيّم دقتها وأبلغ عن أي انحراف عن معايير الجودة. البيانات: ${JSON.stringify(input)}. أجب بـ JSON: {analysis:string,recommendations:array,confidence:number,action_taken:string}`);
      if (!result.data) return { success:false, error:result.error };
      try {
        await pool.query(
          `INSERT INTO agent_execution_logs (agent_name,action,input,output,confidence,status) VALUES ($1,'audit',$2,$3,$4,'completed')`,
...
export { agent as qualityAuditor };
