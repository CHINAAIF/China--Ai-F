/**
 * content_quality_agent | layer: content
 * منطق Groq حقيقي — يستخدم safe-json.js
 */
import dotenv from 'dotenv'; import { getPool, generateDbToken } from './../../lib/db.js';
const pool = getPool('main', generateDbToken('agents/content/content_quality_agent.js'));;
...
class ContentQualityAgent {
  constructor() {
    this.name = 'content_quality_agent';
    this.layer = 'content';
    this.status = 'active';
  }
  ...
  async run(input = {}) {
    const prompt = `Score and improve quality of AI intelligence content.\n\nInput: ${JSON.stringify(input)}\n\nRespond ONLY with JSON: {"quality_score":0,"issues":[],"improvements":[],"confidence":85}`;
...
export const contentQualityAgent = new ContentQualityAgent();
export default contentQualityAgent;
