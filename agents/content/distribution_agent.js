/**
 * distribution_agent | layer: content
 * منطق Groq حقيقي — يستخدم safe-json.js
 */
import dotenv from 'dotenv'; import { getPool, generateDbToken } from './../../lib/db.js';
const pool = getPool('main', generateDbToken('agents/content/distribution_agent.js'));;
...
class DistributionAgent {
  ...
  async run(input = {}) {
    const prompt = `Plan content distribution across channels for maximum intelligence reach.\n\nInput: ${JSON.stringify(input)}\n\nRespond ONLY with JSON: {"channels":[],"schedule":[],"estimated_reach":0,"confidence":85}`;
