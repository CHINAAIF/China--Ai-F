...
const BASE = '/data/data/com.termux/files/home/downloads/China--Ai-F';
const src  = readFileSync(`${BASE}/index.js`, 'utf8');

// ── إنشاء مجلد routes ────────────────────────────────────────────
if (!existsSync(`${BASE}/routes`)) mkdirSync(`${BASE}/routes`);

// ── استخراج الـroutes لملفات منفصلة ─────────────────────────────
...
router.get('/performance', async (req, res) => {
  try {
    const {rows} = await pool.query('SELECT agent_name,accuracy_score,total_runs,failed_runs,avg_latency_ms,degraded,last_run FROM agent_performance_scores ORDER BY accuracy_score DESC');
...
router.get('/costs', async (req, res) => {
  try {
    const {rows} = await pool.query('SELECT agent_name,SUM(tokens_in+tokens_out) as total_tokens,SUM(cost_usd) as total_cost_usd,COUNT(*) as calls FROM cost_tracking WHERE created_at>NOW()-INTERVAL \'24 hours\' GROUP BY agent_name ORDER BY total_cost_usd DESC LIMIT 20');
...
router.get('/redundancy', async (req, res) => {
  try {
    const {rows} = await pool.query('SELECT function_key,active_agent,failure_count,circuit_open,last_success,last_failure FROM agent_redundancy_map ORDER BY failure_count DESC');
...
writeFileSync(`${BASE}/routes/health.js`, healthRoutes);
writeFileSync(`${BASE}/routes/metrics.js`, metricsRoutes);
writeFileSync(`${BASE}/routes/sovereign.js`, sovereignRoutes);

console.log('✅ routes/health.js created');
console.log('✅ routes/metrics.js created');
console.log('✅ routes/sovereign.js created');
process.exit(0);
