...
// ── Railway Safety: لا شيء يقتل العملية ────────────────────────
...
// ════════════════════════════════════════════════════════════════
// HEALTH — يرد فوراً بدون أي اعتماد على شيء
// ════════════════════════════════════════════════════════════════
...
// ── Debug endpoint — يرجع سجل التهيئة ──────────────────────────
app.get('/api/debug/init', (req, res) => {
  res.json({ ready, steps: initLog, env_keys: Object.keys(process.env).filter(k => !k.includes('KEY') && !k.includes('SECRET')) });
});

// ════════════════════════════════════════════════════════════════
// DATABASE ENDPOINTS — لا تعمل إلا إذا نجح الـpool
// ════════════════════════════════════════════════════════════════
function dbReady(req, res, next) {
  if (!pool) return res.status(503).json({ error: 'DB not initialized', ready });
  req.pool = pool;
  next();
}

app.get('/api/intelligence/geopolitical/:slug', dbReady, async (req, res) => {
  try {
    ...
    const g = geo.rows[0];
    res.json({ model: g.model_slug || req.params.slug, risk_score: g.risk_score, country_of_origin: g.country_of_origin, export_restricted: g.export_restricted, gdpr_compliant: g.gdpr_compliant, can_be_blocked: g.can_be_blocked, blocking_regions: g.blocking_regions || [], government_linked: g.government_linked });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

app.get('/api/intelligence/cost-calculate', dbReady, async (req, res) => {
  try {
...
app.get('/api/intelligence/compare', dbReady, async (req, res) => {
  try {
...
app.get('/api/intelligence/benchmarks', dbReady, async (req, res) => {
  try {
...
app.get('/api/intelligence/safety/:slug', dbReady, async (req, res) => {
  try {
    const model = await pool.query('SELECT id, slug, name FROM models WHERE slug = $1', [req.params.slug]);
...
app.get('/api/sovereign/dashboard', dbReady, async (req, res) => {
  try {
...
app.get('/api/supervision/health', dbReady, async (req, res) => {
  try {
...
app.get('/api/judicial/stats', dbReady, async (req, res) => {
  try {
...
app.get('/api/redundancy/health', dbReady, async (req, res) => {
  try {
...
app.get('/api/performance/scores', dbReady, async (req, res) => {
  try {
...
// ════════════════════════════════════════════════════════════════
// START SERVER FIRST — يرد على Railway فوراً
// ════════════════════════════════════════════════════════════════
app.listen(PORT, '0.0.0.0', () => {
  console.log('TRUNKIA listening on ' + PORT);
  ready = true;

  // ── تهيئة في الخلفية — لا شيء يمنع الرد ────────────────────
  (async () => {
    // 1. DB Pool
    try {
    ...
    console.log('Init complete. Steps: ' + initLog.filter(i=>i.status==='ok').length + '/' + initLog.length);
  })();
});
