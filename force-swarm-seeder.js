...
async function executeSeeding() {
    try {
        // 1. بناء وهيكلة الجداول الأساسية فوراً إذا كانت غائبة لمنع الـ 0 records
        console.log('\n🛠️ [1] التأكد من سلامة الهيكل الإنشائي (Schema Verification)...');
        ...
        `);
        console.log('✅ الجداول الهيكلية مؤمنة وموجودة بنسبة 100%.');

        // 2. مصفوفة الـ 93 وكيل كاملة للحقن الفوري
        console.log('\n📥 [2] جاري صب بيانات الـ 93 وكيل في المستودع...');
        const agents = [
        ...
        for (const agent of agents) {
            await pool.query(
                `INSERT INTO agent_registry (agent_name, agent_layer, status, model_provider)
                 VALUES ($1, $2, 'active', 'groq')
                 ON CONFLICT (agent_name) DO UPDATE SET agent_layer=$2`,
                [agent.name, agent.layer]
            );
            agentCount++;
        }
        console.log(`✅ تم تأكيد وحقن: ${agentCount} وكيل بنجاح في قاعدة البيانات.`);

        // 3. مصادر الاستخبارات
        console.log('\n📡 [3] جاري ربط مصادر الاستخبارات الصينية والعالمية...');
        const sources = [
        ...
        for (const src of sources) {
            await pool.query(
                ...
                [src.name, src.type, src.url, src.lang, src.chinese, src.score, src.chinese ? 3 : 6]
            );
            sourceCount++;
        }
        console.log(`✅ تم ربط وحقن: ${sourceCount} مصدر استخباراتي حي.`);

    } catch (err) {
        console.error('🛑 تعثر الحاقن الآلي أثناء البناء:', err.message);
    } finally {
        await pool.end();
        console.log('\n🚀 [المساعد الداخلي]: انتهى بروتوكول الفرض والسيطرة. جاري قراءة العدادات الآن...\n');
    }
}
